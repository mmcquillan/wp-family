...
function set_post_title( $data , $postarr )
{
    if ( $data['post_type'] == 'family' ) {
        if( isset( $_POST['family_last_name'] ) ) {
            $full_name = rtrim(strtoupper($_POST['family_last_name']) . ', ' . $_POST['family_first_name'] . ' ' . $_POST['family_middle_name']);
            $data['post_title'] = preg_replace( "/\s+/", " ", $full_name);
            $data['post_name'] = strtolower( preg_replace( "/\s+/", "-", str_replace(',', '', $full_name) ) );
        }
    }
    return $data;
}
...
function save_post_family( $family_id, $family ) {
    ...
    if ( $family->post_type == 'family' ) {
        ...
        // save date fields
        if ( isset ( $_POST['family_birth_year'] ) ) {
        ...
        if ( isset ( $_POST['family_marriage_spouse'] ) ) {
